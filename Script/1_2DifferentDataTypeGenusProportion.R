...
#### Main####
# ---->Set file pathway----
setwd("/Users/ching-ya/Desktop/Research")
...
compare_table$color <- ifelse((compare_table$wgs_mean>=13|compare_table$rna_mean>=13),rownames(compare_table),"Abundance<13")
...
ggplot2::ggplot(drawdata,aes(x = type,fill = color)) + 
  ...
  labs(title = "Proportion of each genus in different data type",
       x="Data type",
       y="Proportion")
#Filter abundance < 13 genus
drawdata_remove_low_abundance<- drawdata%>%
  filter(drawdata$color != "Abundance<13")
ggplot2::ggplot(drawdata_remove_low_abundance,aes(x = type,fill = color)) + 
  ...
  ggplot2::labs(title = "Proportion of each genus in different data type",
                subtitle = "Remove abundance < 13 genus",
                x="Data type",
                y="Proportion")
