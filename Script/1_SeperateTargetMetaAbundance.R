...
#### Main####
# ---->Set file pathway----
setwd("D:/Research-new/Data/LUAD")

# ---->Read data----
#Metadata
df_meta <- read.csv("D:/Research-new/Data/Raw/Metadata-TCGA-Kraken-17625-Samples.csv")
#Microbiome abundance
df_kraken <- read.csv("D:/Research-new/Data/Raw/Kraken-TCGA-Voom-SNM-Plate-Center-Filtering-Data.csv")
...
write.csv(meta_wgs,file = 'Metadata_LUAD_WGS.csv',row.names = F)
write.csv(meta_rnaseq,file = 'Metadata_LUAD_RNAseq.csv',row.names = F)
write.csv(filter_wgs,file = "Metadata_LUAD_WGS_138sample.csv",row.names = F)
write.csv(filter_rna,file = "Metadata_LUAD_RNA_138sample.csv",row.names = F)
...
data_filter_rna <- ExtractAbundanceData(df_kraken,meta_rnaseq$X)
data_filter_WGS <- ExtractAbundanceData(df_kraken,meta_wgs$X)

# ---->Write the WGS and RNA abundance file----
write.csv(data_filter_rna,file = "LUAD_RNA_abundance_138sample.csv",row.names = F)
write.csv(data_filter_WGS,file = "LUAD_WGS_abundance_138sample.csv",row.names = F)
write.csv(data_filter,file="LUAD_kraken_voom_snm.csv",row.names = F)
write.csv(data_filter_rna,file = "LUAD_RNA_seq_abundance.csv",row.names = F)
write.csv(data_filter_WGS,file = "LUAD_WGS_abundance.csv",row.names = F)
