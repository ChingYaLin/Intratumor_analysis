...
gene <- read.csv("D:/Research/Data/Redo/log2cpm_clean_gene.csv", header = TRUE,row.names = 1)
gene <-t(gene)
...
for (o in 19:100) {
  ...
  path1 = paste("D:/Research/Data/PermutationSCC/gene_Correlation_SCC_random_",as.character(o),".csv",sep="")
  path2 = paste("D:/Research/Data/PermutationSCC/gene_Correlation_pvalue_random_",as.character(o),".csv",sep="")
  write.csv(df_cor,file=path1,row.names = T)
  write.csv(df_pvalue,file=path2,row.names = T)
}
